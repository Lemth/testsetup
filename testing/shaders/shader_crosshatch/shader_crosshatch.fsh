...
float lookup(vec2 p, float dx, float dy)
{
    ...
    vec4 c = v_vColour;//texture(iChannel0, uv.xy);
...
void main()
{
	...
	float coordX = v_vTexcoord.x / iResolution.x;
	float coordY = v_vTexcoord.y / iResolution.x;
	vec2 dstCoord = vec2(coordX, coordY);
	vec2 srcCoord = vec2(coordX, coordY / ratio);	
	vec2 uv = srcCoord.xy;*/

	vec3 res = vec3(1.0, 1.0, 1.0);
    vec4 tex = v_vColour;//texture(iChannel0, uv);
    ...
    {
		if (mod(v_vTexcoord.x + v_vTexcoord.y, densty) <= width)
		{
#ifdef COLOUR_HATCHES
    ...
    {
		if (mod(v_vTexcoord.x + v_vTexcoord.y - (densty*0.5), densty) <= width)
		{
#ifdef COLOUR_HATCHES
